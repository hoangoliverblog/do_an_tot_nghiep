  ...
  <div class="container">
    <div class="row col-md-4">
        <div class="input-group mb-3">
          <input type="text" id="search_staff" class="form-control" placeholder="{{__('msg.EnterSearchWord')}}" aria-label="Recipient's username" aria-describedby="button-addon2">
        </div>
              ...
              @foreach ($staff as $lu)
               <tr>
                  <th scope="row">{{$lu->id}}</th>
                      ...
                      <td>{{$lu->created_at}}</td>
                      <td>
                      <span><a onclick="return edit()" href="{{route('staff.show',[$lu->id])}}"><i class="fas fa-edit"></i></a></span>
                      <form action="{{route('staff.destroy',[$lu->id])}}" method="POST" onsubmit="return xoa()">
                          @csrf
                          @method('DELETE')
